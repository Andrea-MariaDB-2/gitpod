...
export default function NewProject() {
    ...
    const createProject = async (teamOrUser: Team | User, selectedRepo: string) => {
        ...
        history.push(User.is(teamOrUser)
            ? '/projects' // FIXME
            : `/${teamOrUser.slug}/${repo.name}/configure`);
    }
    ...
    const renderSelectTeam = () => {
        ...
        return (<>
            ...
            <div className="mt-8 border rounded-xl border-gray-100 flex-col" >
                {((user ? [user] : []) as Array<User | Team>).concat(teamsToRender).map((t) => {
                    const name = User.is(t) ? (t.fullName || t.name) : t.name;
                    return <div key={`team-${name}`} className={`w-96 border-b px-8 py-4 flex space-x-2 justify-between dark:hover:bg-gray-800 focus:bg-gitpod-kumquat-light transition ease-in-out group`}>
                        <div className="w-8/12 m-auto overflow-ellipsis truncate">{name}</div>
